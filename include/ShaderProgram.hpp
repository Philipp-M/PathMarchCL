...
public:
  ...
  bool vertexAttribPointer(const std::string &attribName, GLint size, GLenum type, GLsizei stride,
			   const GLvoid *data, bool normalize) const;
...
inline GLuint ShaderProgram::getId() const { return shaderProgramId; }

inline const std::string &ShaderProgram::getName() const { return name; }

inline void ShaderProgram::setName(const std::string &name) { ShaderProgram::name = name; }
